...
**Reading the results:** the page shows your goal once (Edit reopens the form); then the winner model, fallback model and suggested architecture side by side; then "Build X on Y" with four metrics (cost per month, accuracy, made-up answers, response time), each checked against your target; then the suggested architecture in detail (each step of a request, what it takes, why, how to build it). Below that one table lines up the recommendation, its fallback, the cheapest alternative and the other architectures on the same columns, and a short list of what to check before relying on it. Everything else is under Details.
